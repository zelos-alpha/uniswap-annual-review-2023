...
folder_path = '/home/zelos/src/uni_pos_research/result/csv/01_init/price_result'
...
for address, user_transactions in tqdm(df.groupby('address'), desc='Processing Files2'):
    ...
    if total_duration > timedelta(0):
        ...
        total_market_durations.append((address, total_duration, total_overlap_duration, persentage, overlap_num))
    else:
        print("total time is zero!")

# Convert list of durations to DataFrame and save to CSV
total_market_df = pd.DataFrame(total_market_durations, columns=['address', 'total_market_duration','total_overlap_duration','persentage %','overlap_num'])
