...
burn_df = pd.read_csv('/home/zelos/src/UGPCA/result/csv/BM_bull_bear_capmBULL.csv')
...
# plt.scatter(trimmed_df['swap_amount'], trimmed_df['burn_count'], s=7, alpha=0.7,c=np.where(trimmed_df['market_type'] == 'bull', 'yellowgreen', 'crimson'))
...
# plt.title('Spot Volume vs Burn Count of High Alpha Address in Bull Market')


plt.xlabel('Spot Volume')
plt.ylabel('Mint Count')
plt.title('Spot Volume vs Mint Count of High Alpha Address in Bull Market')
...
plt.savefig('/home/zelos/src/UGPCA/result/plot/Spot Volume vs Mint Count CAPM BULL.png')
plt.show()